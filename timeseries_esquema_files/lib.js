...
{var id=div.attr("id");var loadedBanners=mdpi_com_adserver_banners[id]["loadedBanners"];var initialImg=false;if(div.closest(".content__container").length>0){div.closest(".content__container").show();initialImg=true;}
...
if(mdpi_com_adserver_banners[id]["loadedBanners"].length===0){if(div.closest(".content__container").length>0){div.closest(".content__container").remove();}
else{div.remove();}}}
...
{mdpi_com_adserver_banners[id]["counter"]=0;mdpi_com_adserver_banners[id]["loadedBanners"].push({'id':matches[2],'content':content});mdpiComAdserverRotateImg(div);}}}},error:function(data){if(div.closest(".content__container").length>0){div.closest(".content__container").remove();}
else{div.remove();}}});}
...
window.scrollTo(x,y);}
$(document).ready(function(){$('.subMenu > li').bind('mouseover',openSubMenu);$('.subMenu > li').bind('mouseout',closeSubMenu);function toggleSubMenu(){$(this).find('ul').toggle();};function openSubMenu(){$(this).find('ul').css('visibility','visible');};function closeSubMenu(){$(this).find('ul').css('visibility','hidden');};function generateSearchFieldTerm(element)
{var result="";var connector=element.find(".connector");var value=element.find(".search-text");var fields=element.find(".search-field");if("or"===connector.val())
{result+=")|(";}
if(""!==value.val())
{result+="@("+fields.val()+")"+value.val();}
return result;}
$("#q, #authors, #journal, #article_type").on("change keyup",function(e){updateSearchLabelVisibilities()});$("#award-status, #award-type, #award-subject, #award-journal").on("change keyup",function(e){updateAwardSearchLabelVisibilities()});$("#advanced-search").on("click",".search-plus",function(e){e.preventDefault();var currentRow=$(this).closest(".advanced-search-row");var newRow=$("#advanced-search-template").clone();newRow.removeAttr("id");currentRow.after(newRow);$(document).foundation('equalizer','reflow');});$("#advanced-search").on("click",".search-minus",function(e){e.preventDefault();var currentRow=$(this).closest(".advanced-search-row");currentRow.remove();$(document).foundation('equalizer','reflow');});$("#advanced-search").on("change",".connector",function(e){$(this).closest(".advanced-search-row").removeClass("and or").addClass($(this).val());});$("#advanced-search").on("click",".advanced-search-button",function(e){e.preventDefault();var queryString="(";$("#advanced-search").find(".advanced-search-row:not(:first)").each(function(e){queryString+=generateSearchFieldTerm($(this));});queryString+=")";if("()"!==queryString)
{window.location="/search/?advanced="+queryString;}else{window.location="/search/?q=&authors=&journal=&article_type=&search=Search"}});$(document).on('open.fndtn.reveal','.reveal-modal[data-reveal]',function(){$('body').addClass('modal-open');if($(this).hasClass("reveal-modal-menu")){$('body').addClass('modal-open-menu');$('.tab-bar .material-icons').closest('a').toggle();}});$(document).on('close.fndtn.reveal','.reveal-modal[data-reveal]',function(){$('body').removeClass('modal-open');if($(this).hasClass("reveal-modal-menu")){$('.tab-bar .material-icons').closest('a').toggle();setTimeout(function(){$('body').removeClass('modal-open-menu');},100);}});});function updateSearchLabelVisibilities()
{if($(".search-container__advanced").first().is(":visible")){$('#basic_search .search-input-label').show();}
else{if(""==$("#q").val()&&""==$("#authors").val()&&(currentJournalNameSystem==$("#journal").val()||""==$("#journal").val())&&""==$("#article_type").val()){$('#basic_search .search-input-label').hide();}
else{$('#basic_search .search-input-label').show();}}}
function updateAwardSearchLabelVisibilities()
{if("all"==$("#award-status").val()&&"all"==$("#award-type").val()&&"all"==$("#award-subject").val()&&"all"==$("#award-journal").val()){$('#awards_search .search-input-label').hide();}
else{$('#awards_search .search-input-label').show();}}
function openAdvanced(advancedQuery)
{if(!$(".search-container__advanced").first().is(":visible"))
{if($('#journal').length>0){var journal_name=$('#journal').val();update_sections(journal_name);}
...
{var newRow=$("#advanced-search-template").clone();newRow.removeAttr("id");$(".advanced-search-row:last").after(newRow);var newRow=$("#advanced-search-template").clone();newRow.removeAttr("id");$(".advanced-search-row:last").after(newRow);}
else
{advancedQuery=advancedQuery.substring(1,advancedQuery.length-1);var items=advancedQuery.split("@(");var nextOr=false;for(var i=1;i<items.length;i++)
{var newRow=$("#advanced-search-template").clone();var fields=items[i].split(")");var connector="and";var field=fields.shift();var value=fields.join(")");if(nextOr)
{nextOr=false;connector="or";}
if(value.indexOf(")|(",value.length-3)!==-1)
{nextOr=true;value=value.substring(0,value.length-3);}
newRow.removeAttr("id");newRow.addClass(connector);newRow.find('.connector').val(connector);newRow.find('.search-text').val($('<textarea/>').html(value).text());if(field=='title,abstract,keywords,authors,affiliations,doi,full_text,references'){field='all';}
newRow.find('.search-field').val(field);$(".advanced-search-row:last").after(newRow);}}}
else{$(".search-container__advanced").hide();$("#advanced-search").hide();updateSearchLabelVisibilities();$(".js-search-expanded-button-container").children().each(function(){$(this).appendTo($(".js-search-collapsed-button-container"));});$(".js-search-expanded-link-container").children().each(function(){$(this).appendTo($(".js-search-collapsed-link-container"));});$(".js-search-collapsed-button-container").show();$(".js-search-collapsed-link-container").show();}
$(document).foundation('equalizer','reflow');}
function show_hide_directions()
{if(typeof Foundation==='undefined'||Foundation.utils.is_large_up()){$('.direction').show();}
else
{$('.direction').hide();$('.big_direction').hide();}}
function toggle_big_directions()
{$('.big_direction').toggle();$('.direction').toggleClass('direction--active');return false;}
$(document).ready
(function()
...
clicked=false;});};$(".big_direction").clickOff(function(){$('.big_direction').hide();$('.direction').removeClass('direction--active');});$('.direction').bind('mouseover',toggle_big_directions);$(".big_direction").mouseleave(function(){$('.big_direction').hide();$('.direction').removeClass('direction--active');});$('.direction').click(function(){toggle_big_directions();return false;});$('.big_direction').click
(function()
{$('.big_direction').hide();$('.direction').removeClass('direction--active');});function div_toggle(id){$("#div_"+id).toggle();$(document).foundation('equalizer','reflow');}
...
total=parseFloat((amount*+exchange).toFixed(2));if(total>parseFloat((2000*+exchange).toFixed(2))&&counter_amount===0&&globalAlertBigInvoice===0){alert('Please check the "Invoiced Amount" which is bigger than 2000 CHF.');counter_amount++;globalAlertBigInvoice++;}
...
if(!$('#paymentTerms').is(':checked')){$('<div class="error">Please read and agree to the Standard Terms and Conditions of Business of MDPI</div>').insertAfter($('#paymentTerms ~ a'));validated=false;}
...
if(validated){e.preventDefault();if($('#currency_jpy:checked').length==1){var jpyAmount=$('#amount').val();$('#amount').val(Math.round(jpyAmount/100));}
var invoiceID=$("#refno").val();var data=$("#refno").val();var url=$("#refno").data('ajax-url-paid')+'?term='+data;var form=this;$.ajax({url:url,dataType:'json',success:function(results){if(results.length==0){form.submit();return false;}
...
return validated;});});}).call(this);var empty_section_option='<option value="">--</option>';var all_sections_option='<option value="">All Sections</option>';var empty_special_issue_option='<option value="">--</option>';var all_special_issues_option='<option value="">All Special Issues</option>';function fill_options(node,items,empty_option,all_option){var data=$.map(items,function(val,key){return "<option value=\""+val[0]+"\">"+val[1]+"</option>";}).join('');$(node).html(data!==''?all_option+data:empty_option);};function select_option(options,value){options.each(function(){if($(this).attr('value')===value){$(this).attr('selected','selected');}});};function journal_changed(){if($('#journal').length>0&&$(".search-container__advanced").first().is(":visible")){var journal_name=$('#journal').val();update_sections(journal_name);}};function update_sections(journal_name){if(journal_name===''){$('#section').html(empty_section_option).trigger("chosen:updated");$('#special_issue').html(empty_special_issue_option).trigger("chosen:updated");}else{$.getJSON("/journal/"+journal_name+"/get/sections",function(data){var sectionId=$('span[data-section-id]').attr('data-section-id');fill_options('#section',data,empty_section_option,all_sections_option);if(sectionId){select_option($('#section > option'),sectionId);$('span[data-section-id]').remove();}
$('#section').trigger("chosen:updated");update_special_issues(journal_name,sectionId);});}};function update_special_issues(journal_name,section_id){if(null===section_id){section_id='';}
if(''===section_id){section_id=0;}
$.getJSON("/journal/"+journal_name+"/sections/"+section_id+"/get/special_issues",function(data){var specialIssueId=$('span[data-special-issue-id]').attr('data-special-issue-id');fill_options('#special_issue',data,empty_special_issue_option,all_special_issues_option);if(specialIssueId){select_option($('#special_issue > option'),specialIssueId);$('span[data-special-issue-id]').remove();}
$('#special_issue').trigger("chosen:updated");});};(function(){$(function(){var current_url,div_my_query_history,div_save_my_query,showSaveQuery;var awards_all_journals_option='<option value="all">All Journals</option>';$('#journal').change(function(){return journal_changed();});$('#award-subject').change(function(){$.getJSON("/subject/"+$(this).val()+"/get/journals",function(data){fill_options('#award-journal',data,awards_all_journals_option,awards_all_journals_option);var awardJournalNameSystem=$('span[data-award-journal-name-system]').attr('data-award-journal-name-system');if(awardJournalNameSystem){select_option($('#award-journal > option'),awardJournalNameSystem);$('span[data-award-journal-name-system]').remove();}
$('#award-journal').trigger("chosen:updated");updateAwardSearchLabelVisibilities();});});$('#section').change(function(){return update_special_issues($('#journal').val(),$(this).val());});journal_changed();$('#award-subject').change();div_my_query_history=$('#div-my-query-history');div_save_my_query=$('#div-save-my-query');$('#save-my-query').click(function(e){return showSaveQuery();});current_url=$('#currentUrl').val();$('#saveurl_description').on('keydown',(function(_this){return function(e){if((e.keyCode||e.which)===13){return $('#add-search-url').click();}};})(this));$('#add-search-url').click(function(e){var saveButton=$(this);var deleteurl=saveButton.data('deleteurl-url');var description=$('#saveurl_description').val();$('#repeat-warning').hide('slow');if(description===''){}
else{saveButton.val('saving');saveData={'url_desc':description,'url':current_url};$.post(saveButton.data('saveurl-url'),saveData,function(result){var delete_img=div_my_query_history.data('delete-img-url');if(result.success){$('#add-search-url').val('save');div_my_query_history.append('<div class="searchurl remove-filter-container"><a class="savedurl-delete link--red" href="#" data-query-del="'+deleteurl+'?id='+parseInt(result.id)+'" style="vertical-align: top;"><i class="material-icons">remove_circle_outline</i></a><label class="search_refine_label"><a href="'+current_url+'" title="'+description+'">'+description.substring(0,25)+'</a></div>');$('#saveurl_description').val('');saveButton.closest('.reveal-modal').find('.close-reveal-modal').click();}
else{$('#saveurl_description').after('<span id="repeat-warning" style="display:block"><span style="color:red; margin-bottom: 5px;">'+result.message+'</span><span id="delete-warning"><img style="vertical-align:top" id="delete-warning" title="delete" src="'+delete_img+'"></span></span>');$('#delete-warning').click(function(e){return $('#repeat-warning').hide('slow');});setTimeout(function(){return $('#repeat-warning').hide('slow');},6000);$('#add-search-url').val('save');}});}});showSaveQuery=function(){$('#saveurl_description').val('');$('#repeat-warning').hide();div_my_query_history.hide();div_save_my_query.toggle();};$("#div-my-query-history").on("click",".savedurl-delete",function(e){e.preventDefault();var needHide=$(this).parent();$.post($(this).data('query-del'),function(data){if(data.success){needHide.hide();}});});});}).call(this);$(function(){$("#refno").on('input propertychange',function(e){var input=$('.open-invoices-quick-search');var element=this;setTimeout(function(){var data=$(element).val().trim();var url=input.data('ajax-url')+'?term='+data;$.ajax({url:url,dataType:'json',success:function(results){if(results.length==0){return false;}
...
if(results[value].currency=="JPY"){$("#currency_chf").attr('disabled',true);$("#currency_eur").attr('disabled',true);$("#currency_usd").attr('disabled',true);$("#currency_gbp").attr('disabled',true);globalAlertBigInvoice++;}
...
$('#currency_chf').next('label').data("has-post-selected-invoice","true");$('#currency_chf').next('label').data('amount',total);$(this).dialog("close");},No:function(){$(this).dialog("close");$('#refno').val("");$('#payAmount').val(0);}},close:function(event,ui){$(this).remove();}});}});},0);})
$('#payment_reprints').on('submit',function(e){if($('#currency_jpy:checked').length==1){var jpyAmount=$('#amount').val();$('#amount').val(Math.round(jpyAmount/100));}});});(function(){$(function(){$('#quotation_link').click(function(e){e.preventDefault();$("#quotation_wrapper").toggleClass('hide');$(document).foundation('equalizer','reflow');});var momentAddDays=function(moment,days,includeWeekend){var counter=0;while(counter<days){moment=moment.add(1,"days");counter++;if(-1!==$.inArray(moment.format("D-M"),["1-1","25-12","26-12"])){counter--;}
...
return result;}
var get_price=function(word_num,has_manuscript,grammar,layout,plagiarism,level,rate){var price=0;if(grammar){switch(level){case "regular":price+=105+word_num*0.02;break;case "express":price+=175+word_num*0.025;break;case "specialist":price+=180+word_num*0.04;break}}
if(layout){price+=75;}
if(plagiarism){price+=50;}
price*=rate;return price.toFixed(2);};var getEncodedParamsString=function(word_num,grammar,layout,plagiarism,level,subject,currency)
...
params+="&currency="+currency;return encodeURIComponent(base64_encode(params));}
var updateSubmitLink=function(submit)
{var price;var word_num=parseInt($('#word_num').val(),10);var id=$('#manuscript_id').val().match(/\d+/);var currency=$('#currency').val();var rate=$('#currency option:selected').data('rate');var level=null;var subject=null;var grammar=$("#service-grammar").is(":checked");var layout=$("#service-layout").is(":checked");var plagiarism=$("#service-plagiarism").is(":checked");if(id){id=id[0];}
if(grammar){level=$("input:radio[name='level']:checked").val();}
if("specialist"===level){subject=$("#specialist-subject").val();}
var manuscript_id=parseInt(id,10);var susy_host=$('#susy_host').val();if(word_num>0&&(grammar||layout||plagiarism)){if(manuscript_id>0){$.get('/is_under_review/'+manuscript_id,function(data){var has_manuscript=data.exists;var price=get_price(word_num,has_manuscript,grammar,layout,plagiarism,level,rate);$("#price").html(price+" "+currency);$("#duration").html(getDuration(grammar,layout,plagiarism,level));var params=getEncodedParamsString(word_num,grammar,layout,plagiarism,level,subject,currency);if(!has_manuscript){$('#msg').html('No manuscript found!').show();$('#proceed_btn').data('href',susy_host+'user/pre_english/upload'+"?params="+params);}
...
else{var price=get_price(word_num,false,grammar,layout,plagiarism,level,rate);$("#price").html(price+" "+currency);$("#duration").html(getDuration(grammar,layout,plagiarism,level));var params=getEncodedParamsString(word_num,grammar,layout,plagiarism,level,subject,currency);$('#proceed_btn').data('href',susy_host+'user/pre_english/upload'+"?params="+params);if(submit){window.location.href=$('#proceed_btn').data('href');}}}}
