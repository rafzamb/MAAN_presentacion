...
{$(this).prop("checked",status);}});}});$("#js-home-journal-select").on("change",function(e){window.location.assign("/journal/"+$(this).val());});$('body').on('click','#reloadCaptcha',function()
...
{$(this).prop("src",$(this).data("src"));});$(this).find("span").toggle();abstractImagePreview.toggle();$(document).foundation('equalizer','reflow');});$('.popupgallery').each(function(){$(this).magnificPopup({type:'image',delegate:'a',index:2,image:{verticalFit:false},gallery:{enabled:true}});});$(document).on("click",'.js-browse-figures .openpopupgallery',function(){var target=$(this).data('target');$('#'+target+'.popupgallery').magnificPopup('open');});$(document).on("click",'.abstract-image-preview .openpopupgallery',function(){var target=$(this).data('target');var index=$(this).data('imgindex');$('#'+target+'.popupgallery').magnificPopup('open');$('#'+target+'.popupgallery').magnificPopup('goTo',index);});$("table").on("keyup",".filters .filter",function(){setTimeout(function(){$(document).foundation('equalizer','reflow');},200);});$(window).scroll(function(){if($(this).scrollTop()>220){$('.back-to-top').fadeIn(500);}else{$('.back-to-top').fadeOut(500);}});$('.back-to-top').click(function(event){event.preventDefault();$('html, body').animate({scrollTop:0},500);return false;})
$(".image-set-row__thumb-images").on("click","img",function(e){var parentRow=$(this).closest(".image-set-row");parentRow.find(".image-set-row__main-image img").attr("src",$(this).attr("src"));});$(".js-div-toggle").click(function(e){e.preventDefault();$($(this).data("targetdiv")).toggle();});});function handleJournalActiveBasedLinks(journalName,link,event,isPublic,isAcceptingSubmissions,isDiscontinued){var msg="";if(isDiscontinued){msg="<em>"+journalName+"</em> is discontinued: "+link.data("disabledmessage");}
